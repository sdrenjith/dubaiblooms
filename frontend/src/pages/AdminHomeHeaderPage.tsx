...
const HERO_SOURCE_OPTIONS: Array<{ value: HomepageHeroSource; label: string; hint: string }> = [
  {
    value: 'latest',
    label: 'Latest posts',
    hint: 'Hero carousel uses the newest published articles.',
  },
  {
    value: 'featured',
    label: 'Featured stories',
    hint: 'Uses featured articles; falls back to latest if none are featured.',
  },
  {
    value: 'cards',
    label: 'Spotlight cards',
    hint: 'Uses static spotlight cards stored in homepage settings.',
  },
];
...
export function AdminHomeHeaderPage() {
  ...
  const heroSource = resolveHomepageHeroSource(form.homepage);
  ...
  const spotlightsDriveHero = useMemo(
    () => heroSource === 'cards' && heroCardsResolved.some((c) => heroCardHasContent(c)),
    [heroSource, heroCardsResolved]
  );
  ...
  const updateHeroSource = useCallback(
    (next: HomepageHeroSource) => {
      clearStatus();
      setForm((prev) => ({
        ...prev,
        homepage: {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const source = resolveHomepageHeroSource(form.homepage);
        const carousel =
          source === 'latest'
            ? latest.slice(0, 5)
            : featured.length > 0
  ...
  const heroSourceHint = HERO_SOURCE_OPTIONS.find((o) => o.value === heroSource)?.hint;
  const liveHeroHeading =
    heroSource === 'latest'
      ? 'Live homepage hero (latest posts)'
      : heroSource === 'featured'
        ? 'Live homepage hero (featured stories)'
        : 'Live homepage hero (spotlight cards)';
  ...
  return (
    <>
      ...
      <div className="admin-screen-intro">
        ...
        <p className="lede admin-screen-lede">Top bar, ticker, and which content powers the homepage hero.</p>
      </div>
      <form onSubmit={onSubmit}>
        <section className="admin-card admin-card-wide">
          <h2>Homepage hero content</h2>
          <p className="admin-hint">
            Choose what fills the homepage hero carousel. Spotlight cards keep the current static cards;
            featured and latest pull live published articles instead.
          </p>
          <label className="admin-hero-source-label">
            Hero source
            <select
              value={heroSource}
              onChange={(e) => updateHeroSource(e.target.value as HomepageHeroSource)}
            >
              {HERO_SOURCE_OPTIONS.map((opt) => (
          ...
          {heroSourceHint ? <p className="admin-hint">{heroSourceHint}</p> : null}
        </section>
        ...
        <section className="admin-card admin-card-wide admin-hero-live-section">
          <div className="admin-hero-spotlights-head">
            <h2>{liveHeroHeading}</h2>
          </div>

          {heroSource === 'cards' ? (
            <p className="admin-hint">
              {spotlightsDriveHero
                ? 'Spotlight cards are active on the public homepage. Switch to Latest posts or Featured stories to use live articles instead.'
                : 'No spotlight cards with content are saved. The homepage will fall back to featured/latest articles until cards are added or you pick another source.'}
            </p>
          ) : null}

          {heroSource !== 'cards' ? (
            liveHeroLoading ? (
              <p className="admin-hint">Loading stories…</p>
            ) : liveHeroStories.length === 0 ? (
              <p className="admin-hint">
                No stories found. Mark articles as featured or publish stories for the hero carousel.
              </p>
            ) : (
                  ...
                  return (
                    <article key={article._id} className="admin-tile-card admin-hero-live-card">
                      ...
                      <header className="admin-tile-card-header">
                        ...
                        <span className="admin-tile-card-eyebrow">
                          {(article.category?.name || 'Story').toUpperCase()}
                        </span>
                      </header>
